          ...
          {sortedEvents.map((value, index) => (
            <div
              ...
              className={`card p-4 items-left relative flex flex-col justify-between rounded-2xl xl:gap-8 max-sm:gap-2  max-lg:gap-4 md:gap-4 sm:gap-4  
               max-sm:p-4  
             lg:max-xl:p-10 sm:max-md:p-6  md:max-lg:p-8  xl:p-16 lg:max-xl:bg-amber-400`}
            >
              <div className="font-bold text-wrap sm:text-2xl lg:text-4xl  lg:max-xl:text-2xl sm:max-md:text-xl  md:max-lg:text-2xl   ">{value['Event Title']}</div>
              <div className="sm:text-md text-md max-sm:text-sm
              sm:text-2xl lg:text-xl  lg:max-xl:text-xl sm:max-md:text-sm  md:max-lg:text-md
              lg:line-clamp-2 md:line-clamp-2 sm:line-clamp-2 max-sm:line-clamp-6
              ">{value['desrip']}</div>
            </div>
          ))}
        </div>
